...
// Utility functions for DiscussionSegment
...
/**
 * Change the Segment obtained from the OpenAI API to DiscussionSegment and add it.
 */
  ...
  if (newSegment.timestamp != undefined && newSegment.texts != undefined) {
    ...
    if (result.length > 0) {
      ...
      if (currentNeedToEscape || lastNeedToEscape || lastSegmentEnded) {
        ...
        result.push(..._splitEndedTextsToSegment(result.pop()!)); // lastSegment is existed
      }
    } else {
      result.push(..._splitEndedTextsToSegment(target));
    }
...
/**
 * Split the text ending with a period into a MinuteSegment
 * @param target
 * @returns
 */
...
// # manipulate text in DiscussionSegment
