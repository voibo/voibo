...
class PostPCMWorkletProcessor extends AudioWorkletProcessor {
  ...
  constructor(options) {
    ...
    this.#sliceSamples = Math.trunc(timeslice * sampleRate * this.#channels / 1000);
  ...
  process(inputs, outputs, parameters) {
    ...
    if (this.#currentFrame < 0) {
      this.#currentFrame = currentFrame - this.#firstFrame - (this.#byteOffset / 4);
    }
...
export class CaptureClient {
  ...
  async start() {
    ...
    await this.#context.audioWorklet.addModule(
      this.#modulePath ? this.#modulePath : this.#moduleURL
    );
    ...
    worklet.port.onmessage = (ev: MessageEvent<any>) => {
      // pcm の中身 ArrayBuffer。 （float32bit ⇒ 4byte毎に１サンプル）
      const [pcm, sampleRate, frame] = ev.data;
      this.#post(pcm, this.#channels, sampleRate, this.#offset + frame);
    };
  ...
  async #post(
    ...
    try {
      this.#handleMessage(buffer, channels, sampleRate, offset);
    } catch (err: unknown) {
